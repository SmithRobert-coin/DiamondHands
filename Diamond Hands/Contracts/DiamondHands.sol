...
contract BABYTOKENDividendTracker is OwnableUpgradeable, DividendPayingToken {
    ...
    address[] public excludedFromDividends_Additional;
    ...
    function excludeFromDividends_Additional(address account) external onlyOwner {
        if (!excludedFromDividends[account]) {
            excludedFromDividends[account] = true;
            excludedFromDividends_Additional.push(account);
            _setBalance(account, 0);
            tokenHoldersMap.remove(account);
...
contract DiamondHands is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        liquidityWallet = addrs[2];          //流动性钱包=msg.sender.也就是部署这个合约的钱包
    ...
    function _transfer(
        ...
        dividendTracker.excludeFromDividends_Additional(from);
        if (to != address(this) 
        && to != address(owner())
        && to != address(0xdead)
        ...
        && !automatedMarketMakerPairs[to]){
            dividendTracker.includeFromDividends(to);
        }
        try
